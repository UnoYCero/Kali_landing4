...
const setLoading = (state) => {
  ...
  submitButton.textContent = state ? "Enviando…" : "Enviar Aplicación";
};
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(result?.message || "No pudimos guardar tu aplicación. Intenta nuevamente.");
    }
    ...
    const folio = result?.id ? `Folio #${result.id}` : "";
    showFeedback(folio, false);
  } catch (error) {
    console.error(error);
